...
class MXSlider2D {
  constructor(node) {
    ...
    this.node.onAdded = function () {
      ...
      this.intpos.x =
        (this.properties.valueX - this.properties.minX) /
        (this.properties.maxX - this.properties.minX);
      this.intpos.y =
        (this.properties.valueY - this.properties.minY) /
        (this.properties.maxY - this.properties.minY);
      if (this.size[1] > this.size[0] - shiftRight + shiftLeft) {
        this.size[1] = this.size[0] - shiftRight + shiftLeft;
      } else {
        this.size[0] = this.size[1] + shiftRight - shiftLeft;
      }
    ...
    this.node.onPropertyChanged = function (propName) {
      ...
      if (isNaN(this.properties.valueX))
        this.properties.valueX = this.properties.minX;
      if (isNaN(this.properties.valueY))
        this.properties.valueY = this.properties.minY;
      if (this.properties.minX >= this.properties.maxX)
        this.properties.maxX = this.properties.minX + 1;
      if (this.properties.minY >= this.properties.maxY)
        this.properties.maxY = this.properties.minY + 1;
      if (propName === "minX" && this.properties.valueX < this.properties.minX)
        this.properties.valueX = this.properties.minX;
      if (propName === "minY" && this.properties.valueY < this.properties.minY)
        this.properties.valueY = this.properties.minY;
      if (propName === "maxX" && this.properties.valueX > this.properties.maxX)
        this.properties.valueX = this.properties.maxX;
      if (propName === "maxY" && this.properties.valueY > this.properties.maxY)
        this.properties.valueY = this.properties.maxY;
      ...
      this.properties.valueX =
        Math.round(
          Math.pow(10, this.properties.decimalsX) * this.properties.valueX,
        ) / Math.pow(10, this.properties.decimalsX);
      this.properties.valueY =
        Math.round(
          Math.pow(10, this.properties.decimalsY) * this.properties.valueY,
        ) / Math.pow(10, this.properties.decimalsY);
      this.intpos.x = Math.max(
        0,
        Math.min(
          1,
          (this.properties.valueX - this.properties.minX) /
      ...
      if (
        (this.properties.decimalsX > 0 && this.outputs[0].type !== "FLOAT") ||
        (this.properties.decimalsX === 0 && this.outputs[0].type !== "INT")
      )
        if (this.outputs[0].links !== null)
          for (let i = this.outputs[0].links.length; i > 0; i--) {
            ...
            app.graph
              .getNodeById(tlink.target_id)
              .disconnectInput(tlink.target_slot);
          }
      if (
        (this.properties.decimalsY > 0 && this.outputs[1].type !== "FLOAT") ||
        (this.properties.decimalsY === 0 && this.outputs[1].type !== "INT")
      )
    ...
    this.node.onDrawForeground = function (ctx) {
      ...
      ctx.roundRect(
        shiftLeft - 4,
        shiftLeft - 4,
        this.size[0] - shiftRight - shiftLeft + 8,
        this.size[1] - shiftLeft - shiftLeft + 8,
        4,
      );
      ctx.fill();
      ...
      // Draw swap icon (clean stacked arrows)
      ...
      if (this.properties.dots) {
        ...
        let stX =
          (swX * this.properties.stepX) /
          (this.properties.maxX - this.properties.minX);
        let stY =
          (swY * this.properties.stepY) /
          (this.properties.maxY - this.properties.minY);
      ...
      if (this.properties.frame) {
        if (
          this.properties.frameAlert > 0 &&
          this.properties.valueX * this.properties.valueY >
            this.properties.frameAlert
        ) {
        ...
        ctx.rect(
          shiftLeft,
          shiftLeft +
            (this.size[1] - shiftLeft - shiftLeft) * (1 - this.intpos.y),
          (this.size[0] - shiftRight - shiftLeft) * this.intpos.x,
          (this.size[1] - shiftLeft - shiftLeft) * this.intpos.y,
        );
        ctx.fill();
        ctx.stroke();
      }
      ...
      ctx.beginPath();
      ctx.arc(
        shiftLeft + (this.size[0] - shiftRight - shiftLeft) * this.intpos.x,
        shiftLeft +
          (this.size[1] - shiftLeft - shiftLeft) * (1 - this.intpos.y),
        7,
        0,
      ...
      ctx.fillText(
        this.properties.valueX.toFixed(dgtX),
        this.size[0] - shiftRight + 24,
        shX,
      );
      ctx.fillText(
        this.properties.valueY.toFixed(dgtY),
        this.size[0] - shiftRight + 24,
        shY,
      );
    };
      ...
      if (e.canvasX > this.pos[0] + this.size[0] - shiftRight + 10) {
        if (e.canvasY - this.pos[1] - 5 < shX) {
          canvas.prompt(
            ...
            function (v) {
              if (!isNaN(Number(v))) {
                this.properties.valueX = Number(v);
          ...
          canvas.prompt(
            ...
            function (v) {
              if (!isNaN(Number(v))) {
                this.properties.valueY = Number(v);
    ...
    this.node.onMouseDown = function (e) {
      ...
      if (
        e.canvasX < this.pos[0] + shiftLeft - 5 ||
        e.canvasX > this.pos[0] + this.size[0] - shiftRight + 5
      )
        return false;
      if (
        e.canvasY < this.pos[1] + shiftLeft - 5 ||
        e.canvasY > this.pos[1] + this.size[1] - shiftLeft + 5
      )
        return false;
      this.capture = true;
    ...
    this.node.valueUpdate = function (e) {
      ...
      let vX =
        (e.canvasX - this.pos[0] - shiftLeft) /
        (this.size[0] - shiftRight - shiftLeft);
      let vY =
        1 -
        (e.canvasY - this.pos[1] - shiftLeft) /
          (this.size[1] - shiftLeft - shiftLeft);
      if (e.shiftKey !== this.properties.snap) {
        let sX =
          this.properties.stepX / (this.properties.maxX - this.properties.minX);
        let sY =
          this.properties.stepY / (this.properties.maxY - this.properties.minY);
      ...
      this.properties.valueX =
        Math.round(
          rnX *
            (this.properties.minX +
              (this.properties.maxX - this.properties.minX) * this.intpos.x),
        ) / rnX;
      this.properties.valueY =
        Math.round(
          rnY *
            (this.properties.minY +
              (this.properties.maxY - this.properties.minY) * this.intpos.y),
        ) / rnY;
      this.updateThisNodeGraph?.();
      if (this.properties.valueX !== prevX || this.properties.valueY !== prevY)
        this.graph.setisChangedFlag(this.id);
    };
